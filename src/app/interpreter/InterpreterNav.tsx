          ...
          <>
            ...
            <div className="absolute right-0 top-full mt-1 py-2 w-48 bg-white rounded-xl border border-slate-200 shadow-lg z-50">
              <div className="flex flex-col gap-0.5 px-2">
                <Link href="/interpreter" className={linkClass('/interpreter', true)} onClick={() => setMobileOpen(false)}>{t('dashboard')}</Link>
                <Link href="/interpreter/profile" className={linkClass('/interpreter/profile')} onClick={() => setMobileOpen(false)}>{t('profile')}</Link>
                <Link href="/interpreter/history" className={linkClass('/interpreter/history')} onClick={() => setMobileOpen(false)}>{t('billing')}</Link>
